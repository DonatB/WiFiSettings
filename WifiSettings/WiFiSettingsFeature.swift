...
class WiFiSettingsModel {
    ...
    init(foundNetworks: [Network], isOn: Bool = true, selectedNetworkID: Network.ID? = nil) {
    ...
    private func configure(
        ...
        switch item {
            ...
            case let .selectedNetwork(networkID):
                guard let network = model.foundNetworks.first(where: { $0.id == networkID }) else { return }
                configureNetwork(
